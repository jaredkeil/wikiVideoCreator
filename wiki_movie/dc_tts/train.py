...
class Graph:
    def __init__(self, num=1, mode="train"):
        ...
        if mode=="train":
            ...
            # self.L = tf.Variable(tf.ones(shape=(None, None), dtype=tf.int32))
            # self.mels = tf.Variable(tf.ones(shape=(None, None, hp.n_mels), dtype=tf.float32))
            # self.prev_max_attentions = tf.Variable(tf.ones(shape=(None,), dtype=tf.int32))
            ### tf v1:
