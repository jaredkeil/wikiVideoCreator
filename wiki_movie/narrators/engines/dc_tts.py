...
def recombine_wavs(script, sample_dir, audio_dir):
    ...
    for sd in script:
        ...
        AudioSegment.from_wav(
            ...
            ).export(out_f=os.path.join(audio_dir, f'{name}_header.mp3'),
                     format='mp3',
                     codec='libmp3lame')

        if sd['text'] or sd['level'] == 0:
            audio_seg = AudioSegment.from_wav(
                os.path.join(sample_dir, f'{ct+1}.wav'))

            for i in range(ct + 2, ct + n):
                audio_seg += AudioSegment.from_wav(
                    os.path.join(sample_dir, f'{i}.wav'))

            audio_seg.export(out_f=os.path.join(audio_dir, f'{name}_text.mp3'),
                             format='mp3',
                             codec='libmp3lame')
        ct += n
...
def process_text_dctts(script, file_path):
    """
    Chunk article text into lines within predefined character limit of DC_TTS.
    ...
    with open(file_path, 'w') as sf:
        ...
        for section in script:
            ...
            section['n_segments'] = len(s_chunks)  # used in recombining later
