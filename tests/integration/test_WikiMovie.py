...
from wiki_movie.video.movie_maker2 import DATA_DIR
...
class WikiMovieTest(TestCase):
    ...
    def test_skip_overwrite_images(self):
        ...
        with mock.patch.object(img_dl, 'find_and_download') as fd:
            self.w_movie.make_movie(overwrite_images=False, overwrite_video=False, overwrite_speech=False)
    ...
    def test_skip_overwrite_speech(self):
        ...
        with mock.patch.object(narrator, 'make_narration') as mn:
            self.w_movie.make_movie(False, False, False)
    ...
    def test_linux_format(self):
        ...
        with mock.patch('wiki_movie.narrators.engines.sys_tts.save_linux') as sl:
            w_movie.make_movie(False, True, False)
            print(sl.call_args_list)

            expected_file_name = str(DATA_DIR / 'audio' / 'Condyle' / 'Condyle_header')

            sl.assert_called()
            sl.assert_any_call(message='Condyle', voice='', rate='', file_name=expected_file_name)
